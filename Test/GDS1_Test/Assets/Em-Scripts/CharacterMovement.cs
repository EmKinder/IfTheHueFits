        ...
        if (Input.GetMouseButtonDown(0) && canHit == true && SceneManager.GetActiveScene() != SceneManager.GetSceneByBuildIndex(0))
        {
                ...
                || asw.GetAmmoType() == "Purple" && ac.getAmmoCount("Purple") > 0)
            {
                ...
                if (groundPlane.Raycast(cameraRay, out rayLength))
                {
                    ...
                    transform.rotation.Set(0.0f, rotTransform.rotation.y, rotTransform.rotation.w, rotTransform.rotation.w);
                    transform.Rotate(new Vector3(0.0f, this.gameObject.transform.position.y, 0.0f));
                }
